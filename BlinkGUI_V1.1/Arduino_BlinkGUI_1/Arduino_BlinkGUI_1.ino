...
} // Get Delay Time Loop function

int getBlinkLoopNum(){
  Serial.print("In the blink loop fuction");
  String numLoopString;
  int numLoopInt = 0;
  
  delay(2000); // Need to wait for pyserial send time 
  numLoopString = Serial.readString();
  numLoopInt = numLoopString.toInt();
  return numLoopInt;
}// Get blink index function
...
if(Serial.available()> 0){ 
    ...
    userInput = Serial.read();               // read user input
      ...
      if(userInput == 'b'){
        ...
        // Use this function to get uset input on number off loops to run
        //int numLoops = getBlinkLoopNum()
